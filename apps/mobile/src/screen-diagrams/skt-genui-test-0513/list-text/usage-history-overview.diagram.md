...
- reference pack: `SKT GenUI Test 0513 / node 12172:5881 / frame index 01`
- figma source: `SKT GenUI Test 0513` / exact frame `리스트_이용내역` (`12172:5882`, `393×852`)
- figma verification: Figma metadata provided for `12172:5882`; name `리스트_이용내역`, type `FRAME`, bounds `393×852`
- source section: `리스트_텍스트` (`12172:5881`, `2365×1052`)
- route: `reference-only/not-an-implementation-route`
- screen title: `이용내역`
- pattern: `list`
- source confidence: `provided-figma-metadata + frame-geometry-summary`
...
- frame geometry:
  - `Statusbar+Header`: `12172:5901`, header rail
  - `Content`: `y=107`, `height=601`
  - `Local_Summary`: `y=0`, `height=125`; child card `x=12`, `y=16`, `width=369`, `height=97`
  - first `Pagestack`: `y=125`, `height=236`; `ContentsTitle`, `Local_Info`, `Divider 1px`, `Local_Info`
  - section divider: `12172:5893`, `y=361`, `height=4`
  - second `Pagestack`: `y=365`, `height=236`; same row pattern
- wireReference:
  - source: `apps/mobile/src/screen-diagrams/skt-genui-test-0513/list-text/usage-history-overview.diagram.md`
...
```txt
┌─AppScreen 393×852─────────────────────────────────────┐
├─Header Statusbar+Header / 12172:5901──────────────────┤
│ 9:41                                      ▮▮▮  wifi ▰ │
│                                                        │
│   ‹   이용내역                                        │
├─Content y=107 h=601───────────────────────────────────┤
...
│   └──── Local_Summary card x=12 y=16 w=369 h=97 ───┘   │
│                                                        │
...
│   쿠폰 적립  │  2026.01.30                           │
│                                                        │
├══Divider 4px / 12172:5893 / y=361 h=4═════════════════┤
│                                                        │
...
- ognBoundaryDecision: reference-only section boundary maps to one summary shortcut OGN candidate once real policy/source config exists
...
- layoutContract:
  ...
  - structure: `Local_Summary` section with one rounded card at `x=12`, `y=16`, `width=369`, `height=97`
  - alignment: each shortcut is centered in its cell; cells share equal width across the card
  - density: compact card inside a `125px` summary zone, followed immediately by preview sections
...
- ognBoundaryDecision: reference-only section boundary maps to one point-preview OGN candidate once real policy/source config exists
- layoutStrategy: first `Pagestack` at `y=125`, `height=236`, with `ContentsTitle`, first `Local_Info`, `Divider 1px`, second `Local_Info`
- layoutContract:
...
- ognBoundaryDecision: reference-only section boundary maps to one discount-preview OGN candidate once real policy/source config exists
- layoutStrategy: `12172:5893` section divider at `y=361`, `height=4`, then second `Pagestack` at `y=365`, `height=236`
- layoutContract:
...
- Use exact Figma section `12172:5881` (`리스트_텍스트`, `2365×1052`) and frame `12172:5882` (`리스트_이용내역`, `393×852`).
- Preserve header/content geometry: `Statusbar+Header` node `12172:5901`; `Content` begins at `y=107` and has `height=601`.
- Preserve `Local_Summary` geometry: section `y=0`, `height=125`; child card `x=12`, `y=16`, `width=369`, `height=97`.
- Preserve both `Pagestack` regions as `236px` preview sections: first at `y=125`, second at `y=365`.
- Preserve the single visible 4px divider `12172:5893` at `y=361`, `height=4` between point preview and discount preview.
