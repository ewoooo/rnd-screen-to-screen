...
- figma verification: Figma tool exact-node inspection succeeded for `14243:28863`; name `리스트_할인내역`, type `FRAME`, bounds `393×1062`
...
- wireReference:
  ...
  - matchedParts: exact reference frame for summary card, period selector, chip filters, month grouping, dated transaction rows, row dividers
...
- patternDecision:
  ...
  - reason: Figma shows a card surface that owns title/value grouping, padding, and radius; it is not a plain heading or row
...
- layoutContract:
  ...
  - density: comfortable card padding with whitespace below before the list header
...
- patternDecision:
  ...
  - reason: `최근 3개월` is a list-range control attached to the header, not a separate chip or bottom sheet trigger in the visible frame
...
- layoutContract:
  ...
  - density: compact row with clear separation from summary above and filters below
...
- patternDecision:
  ...
  - reason: chips express discount categories separately from the period selector
...
- layoutContract:
  ...
  - density: compact controls with chip gaps, not tab underline spacing
...
- patternDecision:
  ...
  - reason: `2026.01` groups dated rows and does not appear as a chip, card, or section title
...
- patternDecision:
  ...
  - reason: rows are the primary list pattern and encode merchant, discount category, date, and amount in a stable two-column layout
...
- Use exact Figma frame `14243:28863` (`리스트_할인내역`, `393×1062`); do not infer from sibling list frames.
...
- Preserve selected chip state on `전체` and keep chips as category filters, not tabs.
- Preserve month label as lightweight date-group text.
