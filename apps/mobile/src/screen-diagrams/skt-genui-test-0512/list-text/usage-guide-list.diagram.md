...
- verified Figma SOT node: `14243:28891`
- verified Figma SOT node name: `리스트_이용안내`
- verified Figma frame: `393×1062`
- figma verification: tool screenshot from exact node `14243:28891` succeeded on 2026-05-17
...
- wireReference:
  ...
  - matchedParts: exact Figma reference frame `리스트_이용안내` (`14243:28891`) preserved as this reference-only pack entry
...
│   ‹   이용안내                                        │
├─Content(scroll)───────────────────────────────────────┤
│ [tabs | tab-navigation | top-content]                  │
│   자주 묻는 질문     멤버십 회원 안내     T멤버십 살펴보기 │
│   ━━━━━━━━━━━                                          │
│                                                        │
│ [topicFilters | chip-filter-row | horizontal-scroll]   │
│   ┌──────┐ ┌──────┐ ┌────────┐ ┌────────┐ ┌────┐     │
│   │ BEST │ │회원가입│ │멤버십 혜택│ │멤버십 신청│ │통합│     │
│   └─sel──┘ └──────┘ └────────┘ └────────┘ └────┘     │
│                                                        │
│ [search | list-search-control | field]                 │
│   ┌────────────────────────────────────────────────┐   │
│   │ 검색                                        ⌕  │   │
│   └────────────────────────────── rounded search ──┘   │
│                                                        │
...
- patternEvidence:
  ...
  - visibleContent: tab bar `자주 묻는 질문`, `멤버십 회원 안내`, `T멤버십 살펴보기`; first tab selected with underline
...
- layoutContract:
  ...
  - density: compact top-control density; no card surface or section divider
...
- layoutContract:
  ...
  - density: compact filter-control density with no wrapping into a grid
...
- patternEvidence:
  ...
  - visibleContent: rounded search field with placeholder `검색` and trailing search icon
  - sectionBoundary: `none`
  - fieldGrouping: `single`
  - rowSeparators: `none`
...
- layoutContract:
  ...
  - density: medium control height with clear separation from chip filters and list rows
...
- patternEvidence:
  ...
  - visibleContent: accordion list; first item expanded with answer text, later items collapsed, thin row dividers between items
  - sectionBoundary: `none`
...
- layoutContract:
  ...
  - alignment: question text leading, chevron trailing, expanded answer aligned under question text within the same row
  - density: list-row density with enough answer line-height for readable multi-line copy
...
- componentCandidates:
  ...
  - name: `Divider`
    fit: `medium`
    source: `@pxds/cx-components` candidate vocabulary
    reason: can provide thin contents separators inside the accordion list
...
- visibleEvidence: selected tab `자주 묻는 질문`, selected chip `BEST`, additional topic chips `회원가입`, `멤버십 혜택`, `멤버십 신청`, `통합`
...
- visibleEvidence: search field placeholder `검색` and trailing search icon
...
- visibleEvidence: repeated `Q. [T우주] 결제수단을 변경할 수 있나요?`, first row expanded answer, collapsed later rows, contents dividers
...
- Use exact Figma frame `14243:28891` named `리스트_이용안내`; do not infer from sibling list frames.
