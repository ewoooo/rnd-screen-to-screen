...
- figma verification: Figma tool exact-node inspection succeeded for `14243:28846`; name `리스트_T플러스포인트 내역`, type `FRAME`, bounds `393×1062`
...
- wireReference:
  ...
  - matchedParts: exact reference frame for point balance summary, inline gift action, period selector, chip filters, month grouping, point rows, row dividers
...
- patternDecision:
  ...
  - reason: Figma groups primary point value, secondary balances, and gift affordance inside one surface
- layoutStrategy: top summary card with primary label/value, trailing compact action, and secondary label/value rows
- layoutContract:
  ...
  - density: comfortable summary-card padding; secondary rows are compact and subordinate
...
- patternDecision:
  ...
  - reason: visible period range control belongs to the history list header
...
- layoutContract:
  ...
  - density: compact header row separated from summary and filters
...
- patternDecision:
  ...
  - reason: chip labels represent point history types and are visually separate from the period selector
...
- layoutContract:
  ...
  - density: compact chip gaps; no connected segmented surface
...
- patternEvidence:
  ...
  - typography: compact month label
- patternDecision:
  - patternFamily: `date-group-header`
  - decision: keep `2026.01` as a lightweight row-group label
  - reason: the month label groups following dated point rows without a card or divider
...
- patternDecision:
  ...
  - reason: rows encode event, point type, date, and amount in a dense list layout
