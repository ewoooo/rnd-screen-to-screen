...
type StatusData = { tone: StatusTone; title: string; text: string };
type FollowUpData = { title: string; rows: readonly InfoListItem[] };
...
function inferTrailing(label: string | undefined): {
	...
	if (!label) return { kind: "value", tone: "neutral" };
	if (/\d/.test(label)) return { kind: "value", tone: "neutral" };
	if (label.includes("완료")) return { kind: "status", tone: "positive" };
	if (label.includes("처리")) return { kind: "status", tone: "cautionary" };
...
export function BillingArrearsStatusScreen({
	...
	return (
		<AppScreen
			top={<ProgressTopBar title="미납 해소 상태" leading="close" />}
