...
export function NcFullJoinCompleteScreen({
  spec,
}: {
  ...
  const summaryItems = summary.items.map((item) => ({
    id: item.id,
    title: item.label,
    sub: "",
  ...
  return (
    <AppScreen>
      ...
      <AppScreen.Header>
        <ProgressTopBar title="회원가입" leading="close" />
      </AppScreen.Header>
      <AppScreen.Content>
        <FlowHero {...hero} />
...
function readData<T>(spec: RenderableScreenSpecV1, key: string): T {
  ...
  if (!value || typeof value !== "object" || Array.isArray(value)) {
    throw new Error(`nc-full-join-complete spec missing data.${key}`);
  }
  return value as T;
}
