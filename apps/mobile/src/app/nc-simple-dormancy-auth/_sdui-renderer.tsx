...
type MethodData = { label: string; items: readonly AuthMethodItem[] };
type CodeData = {
	label: string;
	placeholder?: string;
	helperText?: string;
...
export function NcSimpleDormancyAuthScreen({
	spec,
}: {
...
function readData<T>(spec: RenderableScreenSpecV1, key: string): T {
	...
	if (!value || typeof value !== "object" || Array.isArray(value)) {
		throw new Error(`nc-simple-dormancy-auth spec missing data.${key}`);
	}
	return value as T;
}
