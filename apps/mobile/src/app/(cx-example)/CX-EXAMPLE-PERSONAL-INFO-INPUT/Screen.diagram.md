...
- source: `Figma`
- pattern: `form`
- frame: `SKT GenUI Test 0513 / 상세_정보 입력_인풋` (`393x1130`)
...
- wireReference:
  ...
  - matchedParts: `AppScreen(headerPreset="form-entry")`, header app bar, section divider bands, text sections, field stacks, fixed primary CTA.
  - intentionalDifferences: current screen adds an intro `TitleMain("개인정보 입력")` section above the first divider and uses the implemented 1130px proof frame.
...
│ [phone | readonly-field-section | section-divider]        │
│ 기기변경 휴대폰 번호                                      │
│ ┌──────────────────────────────────────────────────────┐   │
...
- patternEvidence:
  ...
  - typography: `TitleMain`, leading form intro title, no row controls.
- patternDecision:
...
- layoutStrategy:
  ...
  - stack: vertical title-only section.
  - alignment: leading
  - wrapping: title may wrap by `TitleMain`; no body content.
  - overflow: normal content scroll.
- layoutContract:
  - role: form intro
  - structure: title-only `PageStackContents`
  - alignment: leading
  - density: matches implemented intro spacing
  - wrapping: title is the only text block
  - distortionRisk: adding a body wrapper or card would create a section that does not exist in the screen.
- componentCandidates:
  - name: `PageStackContents(title=TitleMain)`
    source: `current Screen.tsx`
    fit: strong
    reason: directly preserves the implemented title-only intro.
    risk: none when content slot remains empty.

### [phone]

- patternEvidence:
  - wireSemanticTag: `[phone | readonly-field-section | section-divider]`
...
- layoutContract:
  ...
  - density: section divider separates it from intro and auth sections
  - wrapping: field value does not wrap in the proof state
  - distortionRisk: replacing disabled field with text row loses field-state evidence.
- componentCandidates:
  - name: `PageStackContents + TitleSection + SectionItem + TextField(disabled)`
    source: `current Screen.tsx`
    fit: strong
    reason: preserves title, field state, and section padding.
...
| --- | --- | --- | --- | --- | --- | --- |
| `CX-PERSONAL-INTRO` | current `Screen.tsx` | structural-only | structural-only | `intro` | none; component proof screen | Preserve title-only intro above first section divider. |
| `POL-MBR-INFO-002-08` | policy-core | `POL-MBR-INFO-002-08` | structural-only | `phone` | none; component proof screen | Preserve disabled phone field. |
...
| `CX-PERSONAL-ADDRESS` | Figma/detail proof | structural-only | structural-only | `address` | none; component proof screen | Preserve three-field address stack with inline address action. |
| `CX-PERSONAL-HOME-AREA` | Figma/detail proof | structural-only | structural-only | `homeArea` | none; component proof screen | Preserve checkbox + mirrored address fields in one stack. |
| `CX-PERSONAL-EMAIL` | Figma/detail proof | structural-only | structural-only | `email` | none; component proof screen | Preserve one typed email field. |
| `CX-PERSONAL-ACTION` | current `Screen.tsx` | structural-only | structural-only | `actions` | none; component proof screen | Preserve fixed bottom primary CTA. |
