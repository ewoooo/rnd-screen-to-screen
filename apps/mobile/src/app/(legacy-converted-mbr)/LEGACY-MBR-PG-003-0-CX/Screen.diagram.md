...
- referenceSearch:
  - `apps/mobile/src/app/(cx)/CX-EXAMPLE-COMPLETE-PLAN-CHANGE/Screen.diagram.md`: useful latest-spec complete diagram structure, but less exact because it has no section dividers or revoke notice.
  - `apps/mobile/src/app/(cx)/CX-EXAMPLE-COMPLETE-ACTIVATION/Screen.diagram.md`: useful complete hero and bottom-action evidence, but rejected as primary because its guided action treatment is different.
