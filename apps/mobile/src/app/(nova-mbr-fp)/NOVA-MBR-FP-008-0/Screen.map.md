...
> Step 2B Thin Map. Owns policy meaning, source coverage, state/CTA meaning, and governance selection only. No layout, spacing, component, or route implementation decisions.

Migration source: `apps/mobile/src/app/(nova-mbr-fp-legacy)/NOVA-MBR-FP-008-0/Screen.map.md`.
Policy meaning, coverage, copy, and governance are inherited unchanged. Only the
route group changes (`nova-mbr-fp-legacy` → `nova-mbr-fp`); the screen ID,
dormant-release flow context, and the `NOVA-MBR-FP-009-0` transition target are
identical.
...
- pattern: `form`
- transition: 본인인증 완료 → `NOVA-MBR-FP-009-0` (SB 화면전환표 기준)
...
Coverage verdict: auth selection and auth request rules are GREEN — all eleven
referenced AUTH policy IDs are present in policy-core. Dormant-release context is
yellow because policy-core AUTH source text is written for membership signup,
while the current screen/config attaches the same shared auth policies to the
dormant-release flow. This map applies the common identity-verification gate and
marks dormant-specific copy as flow context, not a new policy. The yellow items
are flow context and generic system-error copy only; they do not block the green
policy-backed implementation requirements.
...
- `Screen.config.ts generation.ognIds`: both OGN IDs appear in this map; every mapped requirement carries at least one OGN ID.
- Build-time selection fields (intro, authSelect, authRequest, actions) are traceable to the rows above; thin map does not assert Build JSON.
