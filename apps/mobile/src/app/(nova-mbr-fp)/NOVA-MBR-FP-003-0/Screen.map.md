...
Step 2B Thin Map SOT. This file owns policy meaning, coverage, user copy, states,
CTA meaning, and governance refs only. It does not own layout, spacing,
component selection, or route registration.

Migration source: `apps/mobile/src/app/(nova-mbr-fp-legacy)/NOVA-MBR-FP-003-0/Screen.map.md`.
Policy meaning, coverage, copy, and governance are inherited unchanged. Only the
route group changes (`nova-mbr-fp-legacy` → `nova-mbr-fp`); the screen ID and
transition target are identical.
...
Coverage verdict: GREEN. All eleven referenced AUTH policy IDs are present in
`packages/policy-core/policies/MBR/AUTH/`. The only non-policy item is the
external-agency error (FP-003-E4), isolated as SB-only generic system error.
...
- transition: successful identity verification moves to `NOVA-MBR-FP-004-0` (SB 화면전환표 기준).
- configTrace:
...
- All `Screen.config.ts generation.policyRefs` are mapped to policy-backed requirements; every mapped requirement carries at least one OGN ID.
