...
- wireReference:
  ...
  - intentionalDifferences: legacy PG-003-0 folds method list, code field, timer, and request controls into a single `ogn-mbr-list-cell-auth-method`; this screen separates policy ownership into two OGNs — `ogn-mbr-auth-select` (method choice list) and `ogn-mbr-auth-request` (code field, timer, resend, confirm) — and adds policy-bound expiry/limit/block notice states inside `ogn-mbr-auth-request`. Intro copy is membership-signup framing (`회원 가입을 위해…`).
  - limitation: reference-only visual structure; policy refs, OGN ids, route, copy, and error states come from `Screen.map.md` and SB. Legacy reference omits the resend cooldown/limit and fail-limit/block notice states that policy requires here.
...
- role: Introduce the membership-signup identity verification step and set the task before the method choice list.
- visibleTitle: `본인인증`
- visibleContent: `회원 가입을 위해 본인인증을 완료해 주세요`
- policy: surfaces `POL-MBR-AUTH-001-01` (회원 가입 시 본인인증 적용) as the step intro
...
- patternDecision:
  ...
  - reason: single title plus one subtitle line below `Header`; no card, divider, or field; matches legacy PG-003-0 intro structure with signup-framed copy.
- ognBoundaryDecision:
  ...
  - decision: keep intro outside policy `ognIds`; it frames `POL-MBR-AUTH-001-01` but does not own auth state or validation
- layoutStrategy:
  ...
  - overflow: `multiline`
- layoutContract:
  - role: open the verification step and frame the task as a signup requirement
  ...
  - distortionRisk: collapsing the intro into the AppBar, or expanding it with full policy prose, breaks the page-header rhythm
- componentCandidates:
...
- deviationReason: `FpPageHeader` is functionally equivalent to the existing `SectionHeaderPage` (`ogn-mbr-section-header-page`); it is re-expressed as FP route-group non-policy chrome only because this screen belongs to a different route-group with separate chrome identifiers — a recognized reuse-equivalent duplication, not a new capability.
- distortionRisk: low; risk appears only if signup intro copy is swapped for legacy step copy or policy prose
...
- patternDecision:
  ...
  - reason: 3 mutually exclusive method rows inside one card surface with 1px row separators; selection state is single-choice. Loading state replaces rows with skeleton (`UXPT_LOD`).
- ognBoundaryDecision:
  ...
  - decision: do not merge with `ogn-mbr-auth-request` even though the legacy reference combines both; this boundary preserves `POL-MBR-AUTH-002-*`
- layoutStrategy:
  - widthTier: `content-361`
  ...
  - wrapping: method labels stay one line (short fixed labels)
...
- ognBoundaryDecision:
  ...
  - decision: keep all verification feedback inside this OGN and out of bottom chrome; do not claim `PageStackContents` for this body section
- layoutStrategy:
  - widthTier: `content-361`
...
- role: Own the fixed verification-complete action; disabled until `ogn-mbr-auth-request` reports a successful verification, then hands off to NOVA-MBR-FP-004-0.
- visibleContent: disabled primary CTA `인증 완료`
- policy: progression gated by the auth-request verification result; completion handoff is SB-defined (NOVA-MBR-FP-004-0)
...
- layoutStrategy:
  ...
  - overflow: none
- layoutContract:
  - role: proceed to the next signup step once verification succeeds
...
- deviationReason: `MbrFpActionBar` is functionally equivalent to the existing `MbrPrimaryCTABar`; it is re-expressed as FP route-group non-policy chrome only because this screen belongs to a different route-group with separate chrome identifiers — a recognized reuse-equivalent duplication, not a new capability.
...
- visibleEvidence: `본인인증`, `회원 가입을 위해 본인인증을 완료해 주세요`
- policyInterpretation: `POL-MBR-AUTH-001-01` establishes identity verification as the membership-signup step task
- OGNInterpretation: chrome `FpPageHeader`; no policy OGN id registered for the intro
- decision: keep as the first content section with signup-framed copy
...
- policyInterpretation: progression is unavailable until `ogn-mbr-auth-request` reports verification success; completion transition to NOVA-MBR-FP-004-0 is SB-defined
...
- Preserve explicit AppScreen rails: `SystemHeader`, `Header` (`‹ 회원 가입`), `Content(scroll)`, and `Bottom(preset="primary-cta")`.
