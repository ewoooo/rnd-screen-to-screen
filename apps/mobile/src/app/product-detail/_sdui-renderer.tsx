...
function renderNode(node: SDUINode, spec: RenderableScreenSpecV1): ReactNode {
  switch (node.type) {
    case "AppScreen": {
      ...
      return (
        <AppScreen>
          ...
          <AppScreen.Header>
            {renderSlotValue(props.top, spec)}
          </AppScreen.Header>
          <AppScreen.Content>
            {node.children?.map((child) => (
              <FragmentNode key={child.id}>
                {renderNode(child, spec)}
              </FragmentNode>
            ))}
          </AppScreen.Content>
          <AppScreen.Bottom>
            {renderSlotValue(props.bottom, spec)}
          </AppScreen.Bottom>
        </AppScreen>
      );
    }
    ...
    case "GlobalNavigationBar": {
      ...
      return (
        <GlobalNavigationBar active={active === "shop" ? "shop" : undefined} />
      );
    }
    case "ProductTopBar": {
      const props = asObject(node.props);
