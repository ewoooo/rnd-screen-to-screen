...
- DoD: `Screen.diagram.html`이 `lifecycle: "thin"` 상태로 build 가능한 계약을 담고, visible DOM과 hidden `#diagram-contract`가 같은 section ID/OGN ID/pattern role을 공유한다. 각 section은 최소 `ognBoundaryDecision`, `layoutContract`, `componentCandidates`, known risk를 가지고, action-bearing section 또는 Bottom CTA 인접 section은 `visualWeightContract`를 가진다. 1차 Diagram draft 이후 `DESIGN_PATTERNS.md`를 다시 열어 Design Pattern Review Gate를 수행하고, `diagram-contract.screenContract.patternRecheck`에 `source`, `result`, `reason`, `changes`를 기록해야 Build로 넘어갈 수 있다.
...
- Design Pattern Review Gate는 1차 Diagram draft 작성 직후, Build 계획 전에 반드시 수행한다. `DESIGN_PATTERNS.md`의 해당 pattern family와 layout/spacing contract를 draft Screen Wire에 대조해 section boundary, divider, CTA placement, content density, field/list/card grouping, state 표현이 맞는지 확인한다. 동시에 Component Composition Gate를 수행해 선택/필드/상태/액션 조합이 pattern slot에 맞는지, `variant` 이름이 아니라 실제 visual weight가 위계를 보존하는지 확인한다. 불일치가 있으면 `Screen.diagram.html`을 먼저 수정하고, 코드로 보정하지 않는다.
- Component Composition Gate는 Content 내부 액션과 Bottom CTA가 같은 화면에 있을 때 필수다. `visualWeightContract`에 allowed/disallowed shape, width tier, height tier, emphasis, proximity, fail condition을 기록하고, Content 내부 full-width pill 액션이 Bottom Primary와 경쟁하면 `componentCandidates.fit`을 `reject` 또는 `risky`로 내려야 한다.
...
- DoD: Diagram의 모든 OGN/슬롯이 코드에 존재하고, 각 OGN 구현이 Step 3의 `ognBoundaryDecision`, layoutContract, visualWeightContract를 보존한다. Render Geometry Evidence에는 viewport, Header/Content/Bottom/CTA bounding box, scroll/client height, overlap/overflow 여부, 주요 policy-bearing text 존재, CTA/secondary action visual hierarchy 판정이 기록된다. Contract Sync 후 `Screen.diagram.html`은 `lifecycle: "synced"`와 `renderEvidence`, `iterationPasses`, `contractSync`를 가진다.
- 메인 에이전트 구현 검수는 실제 렌더링에서 레이아웃 보존 여부를 먼저 본다. `Screen.diagram.html`의 section/slot/stack 배치, 하단 CTA/fixed 영역, scroll/content 영역, 모바일 viewport 줄바꿈, overflow, 겹침, 과도한 여백을 geometry evidence로 확인한다. CTA hierarchy, 버튼 시각 강도, content action과 Bottom action의 경쟁 여부는 geometry만으로 닫지 않고 screenshot/capture 또는 visual review note를 함께 남긴다.
