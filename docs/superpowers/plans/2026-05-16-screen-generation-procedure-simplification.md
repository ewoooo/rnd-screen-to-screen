...
- `ScreenRouteConfig`(`packages/pxds-spec/src/screen.ts:24`)에 `generation?: { source; pattern; policyRefs; ognIds; designDocsChecked }` 가 이미 존재 → 백필 시 `satisfies ScreenRouteConfig` 타입 에러 없음.
