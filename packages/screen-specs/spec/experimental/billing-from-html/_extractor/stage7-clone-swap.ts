 ...
 *
 * 입력:
 *   - packages/screen-specs/spec/active/billing/<route>.json (ScreenSpecV2)
 *   - packages/screen-specs/spec/active/billing/<route>.sdui.json (Renderable)
 *   - packages/screen-specs/spec/active/billing/_pagination/billing-{chk,pay,set}.json
 *   - apps/mobile/src/app/<route>/page.tsx
 *   - apps/mobile/src/app/<route>/_sdui-renderer.tsx
 *
 * 출력:
 *   - packages/screen-specs/spec/experimental/billing-from-html/billing-html-<route>.json
 *   - packages/screen-specs/spec/experimental/billing-from-html/billing-html-<route>.sdui.json
 *   - packages/screen-specs/spec/experimental/billing-from-html/_pagination/billing-html-{chk,pay,set}.json
...
const ACTIVE_BILLING = resolve(REPO_ROOT, "packages/screen-specs/spec/active/billing");
...
function cloneScreenSpecV2(activeId: string, htmlId: string, paginationOut: string) {
	...
	const next = {
		...
		meta: {
			...
			route: `/${htmlId}`,
			policy_doc: HTML_DOC,
			pagination_ref: `packages/screen-specs/spec/experimental/billing-from-html/_pagination/billing-html-${useCase}.json`,
		},
		screen: {
			...src.screen,
			id: htmlId,
...
function cloneSduiSpec(activeId: string, htmlId: string, paginationOutPath: string) {
	...
	const useCase = htmlId.startsWith("billing-html-pay") ? "pay" : htmlId.startsWith("billing-html-set") ? "set" : "chk";
	const newCanonical = `packages/screen-specs/spec/experimental/billing-from-html/_pagination/billing-html-${useCase}.json`;
