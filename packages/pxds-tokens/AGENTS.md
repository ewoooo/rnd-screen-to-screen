...
- 레지스트리 원천은 `registry/wds-token-registry.json`이다. Tokens Studio single-file 포맷(3 Set: `foundation`/`semantic`/`project` + `$metadata.tokenSetOrder` + DTCG `$type`/`$value` 리프 + `{set.path}` alias).
- 값은 JSON에만 산다. TS export는 타입과 이름만 제공하는 얇은 막이다 (`SpacingToken`, `spacingVar`, `semanticSurface`).
