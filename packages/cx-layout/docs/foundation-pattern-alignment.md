# cx-layout foundation/pattern alignment plan

## 목적

`@pxds/cx-layout`은 단순한 화면 wrapper나 scroll utility가 아니라, `DESIGN_FOUNDATION.md`와 `DESIGN_PATTERNS.md`의 물리적 화면 계약을 강제하는 layout runtime이어야 한다.
...
현재 `cx-layout`은 `AppScreen`, `ContentOutlet`, `ContentSection`, `ContentRail`, `BottomSheet`, low-level primitives를 제공한다. 화면 뼈대는 있으나 디자인 SOT의 핵심 규칙을 충분히 강제하지 못한다.
...
기존 pattern guide에는 header가 `StatusBar 59 + AppBar 48 = 107`로 정리되어 있다. Text Section 목업은 `61 + 56 = 117`을 사용한다. 따라서 `cx-layout`은 하나의 숫자를 전역 상수로 박기보다, 화면 역할에 따른 semantic preset을 제공해야 한다. 수치는 preset 내부 contract로 숨기고 route API에는 드러내지 않는다.
...
- `ActionButton` 자체의 시각 구현은 `@pxds/cx-components`가 맡고, `cx-layout`은 action zone 높이와 scroll boundary를 맡는다.
...
- `cx-components`의 bridge 구현과 실제 layout contract 소유 위치가 분리되어, 소비자가 `cx-layout` API로 이동할 수 있다.
...
```txt
@pxds/cx-layout
@pxds/cx-layout/components
@pxds/cx-layout/components/primitives
@pxds/cx-layout/components/compositions
@pxds/cx-layout/components/contents
@pxds/cx-layout/components/chrome
@pxds/cx-layout/components/overlays
@pxds/cx-layout/components/patterns
@pxds/cx-layout/primitives
@pxds/cx-layout/styles.css
```
...
기존 `app-screen`, `patterns`, `overlays`, `bottom-sheet` shim export는 제거한다. 신규 화면과 자동 생성은 책임 단위 export인 `@pxds/cx-layout/components/{chrome,contents,compositions,overlays,patterns,primitives}`를 우선한다.
...
3. `PageStackContents`, `PageStackList`, `ScreenSection`, `SectionDivider`를 `cx-layout`에 추가해 Text Section의 completion/form 화면부터 적용한다.
...
7. `cx-components`의 `PageStackList`는 deprecated bridge로 표시하고 소비자를 `cx-layout`의 `PageStackList`로 이동시킨다.
...
- Text Section의 `PageStackContents -> ContentsTitle -> ContentsSlot -> SectionItem -> inner content` 계층을 `cx-layout`의 공식 Figma-named layout API로 표현할 수 있다.
- `cx-components`의 `PageStackList`는 deprecated bridge로 남고, 신규 소비자는 `cx-layout`의 `PageStackList`를 사용한다.
