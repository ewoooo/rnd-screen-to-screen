export const SCREEN_GENERATOR_RUNTIME_SOURCE = String.raw`
...
async function ensureVariableObjects_p() {
  ...
  try {
    ...
    console.warn("PXDS screen export variable load failed:", error && error.message ? error.message : error);
  }
}
...
async function renderAppScreen_p(spec, parent, ctx) {
  ...
  if (top) await renderScreenNode_p(top, parent, ctx, 0);
  let y = 128;
  for (let i = 0; i < content.length; i++) {
    const node = await renderScreenNode_p(content[i], parent, ctx, y);
    if (node) y += node.height + 4;
  }
  if (bottom) await renderScreenNode_p(bottom, parent, ctx, 704);
  return parent;
}

async function renderScreenNode_p(spec, parent, ctx, y) {
  ...
  return createScreenNode_p(spec, parent, 0);
}
...
async function createScreenNode_p(spec, parent, depth) {
  ...
  const children = spec.children || [];
  for (let i = 0; i < children.length; i++) {
    await createScreenNode_p(children[i], frame, depth + 1);
  }
  return frame;
}

async function generateScreenFrame(SCREEN_SPEC, DS_TOKENS) {
  await ensureVariableObjects_p();
  const root = figma.createFrame();
  root.name = "screen/" + SCREEN_SPEC.id + " · " + SCREEN_SPEC.name;
  root.resizeWithoutConstraints(SCREEN_SPEC.frame.width, SCREEN_SPEC.frame.height);
  root.layoutMode = "NONE";
  root.primaryAxisSizingMode = "FIXED";
  root.counterAxisSizingMode = "FIXED";
  root.clipsContent = false;
  root.setPluginData("pxds-screen-id", SCREEN_SPEC.id);
  root.setPluginData("pxds-screen-route", SCREEN_SPEC.route);
  applyFillRef_p(root, SCREEN_SPEC.frame.background, DS_TOKENS);
  figma.currentPage.appendChild(root);
  await renderScreenNode_p(SCREEN_SPEC.root, root, { data: SCREEN_SPEC.data || {}, tokens: DS_TOKENS }, 0);
  figma.viewport.scrollAndZoomIntoView([root]);
  figma.notify("PXDS screen export: " + SCREEN_SPEC.name);
  return root;
}
`;
