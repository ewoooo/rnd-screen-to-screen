...
type RenderScreenSpecToPageFigmaSpecOptions = {
	registeredComponentIds?: readonly string[];
};

export function renderScreenSpecToPageFigmaSpec(
	spec: RenderScreenSpecLike,
	options: RenderScreenSpecToPageFigmaSpecOptions = {},
): PageFigmaExportSpec {
...
function withSlot(node: RenderSpecNodeLike, slot: PageFigmaSlot): RenderSpecNodeLike {
	return {
		...node,
		props: {
			...(node.props ?? {}),
...
function mapRenderSpecNode(
	...
	const slot = toPageSlot(props._slot);
	const componentId = normalizeComponentId(node.component);
	return {
		id: componentId,
...
function toPageSlot(value: unknown): PageFigmaSlot | undefined {
	if (
		value === "top" ||
		value === "content" ||
		value === "bottom" ||
