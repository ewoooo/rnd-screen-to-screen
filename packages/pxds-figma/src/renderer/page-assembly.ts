...
function collectTransitiveRequiredComponentIds(ctx: AssemblyContext) {
	const specById = new Map(
		ctx.componentSpecs.map((entry) => [entry.componentId, entry.spec]),
	);
	const idBySpecName = new Map(
		ctx.componentSpecs.map((entry) => [entry.spec.name, entry.componentId]),
	);
...
function mapNodeToAssemblyChild(
	...
	if (componentEntry && isLayoutPrimitiveNode(node)) {
		return createLayoutPrimitiveGroup(node, componentEntry.spec, ctx);
	}
	if (componentEntry) return createComponentRef(node, componentEntry.spec, ctx);
...
function createRenderTreeGroup(
	...
	const children = (render.children ?? [])
		.map((child) => mapNodeToAssemblyChild(renderChildToPageNode(child, ctx), ctx))
		.filter((child): child is ComponentSpecChild => Boolean(child));
...
function renderChildToPageNode(
	...
	return {
		...
		slot: toPageFigmaSlot(child.slot) ?? toPageFigmaSlot(child.layout?.slot),
		section: renderSectionToPageSection(child.layout),
		props: {
			...(child.props ?? {}),
...
function toPageFigmaSlot(
	slot: FigmaRendererRenderLayoutContract["slot"] | undefined,
): PageFigmaNodeSpec["slot"] | undefined {
...
function renderSectionToPageSection(
	layout: FigmaRendererRenderLayoutContract | undefined,
): PageFigmaNodeSpec["section"] | undefined {
...
function isLayoutPrimitiveNode(node: PageFigmaNodeSpec) {
	return (
...
function getLayoutMode(node: PageFigmaNodeSpec): ComponentSpecLayout["mode"] {
