...
WDS component re-export, PXDS-owned atoms/domains, component vocabulary registry를 소유한다.
...
- `domains/shared` — 여러 도메인이 함께 쓰는 도메인 독립 조합. `MediaBlock`, `QueryBar`, `FilterTabs`, `FormField`, `SelectField`, form controls, `InfoList`, `SelectableList`, `ConsentList`, `PromoBlock`, `NoticeBlock`, `SectionCard`, `SummaryCard`, `ChipGroup`, CTA bars 등.
...
- 반복되는 WDS 조합은 앱 로컬 shim 대신 이 패키지 `domains/shared`와 `@pxds/pxds-components/shared` 공개 진입점으로 승격한다.
